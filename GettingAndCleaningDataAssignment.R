...
test_subject <-read.csv("UCI HAR Dataset/test/subject_test.txt",sep=" ",header = F)
test_subject <- setNames(test_subject, "subjectnr")

test_X <-read.csv("UCI HAR Dataset/test/X_test.txt",sep="",header = F)
test_X <- setNames(test_X, features$featurename)
test_Y <-read.csv("UCI HAR Dataset/test/y_test.txt",sep="",header = F)
test_Y <- setNames(test_Y, "label")
testdata <- cbind(test_subject, test_Y, test_X)

train_subject <-read.csv("UCI HAR Dataset/train/subject_train.txt",sep="",header = F)
train_subject <- setNames(train_subject, "subjectnr")
train_X <-read.csv("UCI HAR Dataset/train/X_train.txt",sep="",header = F)
train_X <- setNames(train_X, features$featurename)
train_Y <-read.csv("UCI HAR Dataset/train/y_train.txt",sep="",header = F)
